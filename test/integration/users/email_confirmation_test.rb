...
class Users::EmailConfirmationTest < ActionDispatch::IntegrationTest
  ...
  test "confirmation sent" do
    ...
    assert find(class: "alert-danger").has_text?("You have to confirm your account
      before continuing.")
  end
  ...
  test "Email confirmed messasge" do
    visit "/confirmation?confirmation_token=#{@unconfirmedUser.confirmation_token}"
    assert_equal current_path, new_user_session_path
    assert find(class: "alert-danger").has_text?("Your account was successfully
       confirmed.")
  end
  ...
  test "resend confirmation" do
    ...
    fill_in "Email", with: @unconfirmedUser.email
    assert_emails 1 do
      click_on class: "form-control btn-primary"
      assert_equal current_path, new_user_session_path
    end
    assert find(class: "alert-danger").has_text?("You will receive an email with
  ...
  test "change email" do
    ...
    assert find(class: "alert-info").has_text?("A confirmation message for your new
      email has been sent to: #{newEmail} to save changes confirm email first");
  end
end
